    ...
    for line in String::from_utf8_lossy(&nm.stdout).lines() {
        if line.ends_with(':') {
            ...
            if let Some(module) = member.strip_suffix(".o") {
                // SwiftPM also bundles SwiftRs.o into Tauri's archive. Xcode 27
                // internalizes its C ABI exports too; Rust calls them directly.
                let module_name = norm(module);
                in_own_member = module_name == pkg || module_name == "swiftrs";
                continue;
            }
        }
