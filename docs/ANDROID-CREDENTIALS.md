...
2026-09-19：荣耀 MTN-AN00 / Android 16 上上述 7 项 instrumentation 执行通过。修复包覆盖安装成功，冷启动 578 ms；WebView 实测 `read/write/remove` 三种直接存储调用全部被阻止。49 项 Rust 回归、21 项 Node 测试及 4 种移动设备登录文案回归通过（7 项联网 Rust 测试未运行）。另外已验证两个真实平台账号重新登录后，强制结束进程并冷启动，网易云和 QQ 均恢复登录；检查仅记录登录布尔值，未导出凭据。
