...
**当前源码版本：0.9.6（iOS 构建 90601）。** 这一版统一导航、弹窗、主题、歌词和封面的过渡，减少刷新、重复点击与快速切歌时的闪烁，尊重系统「减少动态效果」设置。实现与验证范围见 [动画与界面稳定性](docs/ANIMATIONS.md)。macOS 目标为 macOS 15+ Apple Silicon，iOS 16+ 为开发签名真机版；系统播放组件见 [系统媒体控制](docs/SYSTEM-MEDIA.md)。QQ 接口与下载已迁移到 Rust，应用不再携带 Python。迁移与构建方法见 [Rust 迁移与 iOS](docs/RUST-MIGRATION.md)。
