...
**当前源码版本：0.9.5（构建 90504，封面过渡验收中）。** 本轮修复 macOS / iOS 系统媒体动作的注册时机，接入 Windows SMTC 与 Linux MPRIS，并改善手机歌词面板过渡。macOS 目标为 macOS 15+ Apple Silicon，iOS 16+ 为开发签名真机版；各平台的构建及实机验证状态见 [系统媒体控制](docs/SYSTEM-MEDIA.md)。QQ 接口与下载已迁移到 Rust，应用不再携带 Python。迁移与构建方法见 [Rust 迁移与 iOS](docs/RUST-MIGRATION.md)。
