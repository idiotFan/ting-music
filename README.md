...
- **两平台混编一个歌单**：本机歌单可以把网易云和 QQ 音乐的歌曲放在一起，跨平台加入时按歌名、歌手、专辑、时长核对版本；收藏、本机歌单通过 iCloud Drive 在 Mac 与 iPhone 间自动合并。
...
- 导入本地音频并离线播放（文件授权仅本次会话，本地歌曲暂不能加入本机歌单）；macOS 标题栏与内容区融合。
- 记住上次播放的歌曲、进度与关闭时所在的歌单；桌面三栏布局可拖拽调整列宽。
