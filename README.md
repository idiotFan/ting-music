...
一个小窗口里的网易云音乐 / QQ 音乐第三方播放器。Tauri v2 + TypeScript + Rust，macOS / Windows / Linux / iOS / Android 同一套代码。

![三栏布局：搜索结果、播放器与歌词](docs/assets/lyrics-panel.png)

*截图为 macOS 实机运行画面（0.9.7），账号信息已收敛为首字母。*

## 重点功能

- **双平台同时登录**：网易云（手机号验证码 / 扫码）与 QQ 音乐（QQ / 微信扫码）可以同时在线，各自独立退出；凭据只存在系统钥匙串 / 凭据管理器 / Keystore，不进网页层。
- **两平台 + 本地音乐混编一个歌单**：本机歌单可以把网易云、QQ 音乐和本地文件放在一起，跨平台加入时按歌名、歌手、专辑、时长核对版本；收藏、本机歌单通过 iCloud Drive 在 Mac 与 iPhone 间自动合并（本地文件的归属只留在本机）。
- **按账号权限取最高音质**：标准 / 高品质 / 无损 / 最高可用统一选项，播放区显示接口实际返回的档位与格式；无损、Hi-Res、母带都按当前会员权限提供，不伪装。
- **歌手、专辑一点就到**：搜索分单曲 / 歌手 / 专辑 / 歌单，歌手名就是最佳匹配时置顶直达；歌曲里的歌手名、专辑名可以点进歌手页（热门歌曲、专辑、相似歌手）和专辑页；可同时搜两个平台，同一首歌只留一份。
- **下载即入库、离线优先**：下载完成的歌曲自动出现在「本地」，在线歌单里标「已下载」并直接播放本机文件；整张专辑 / 歌单可排队下载（两首并发、可暂停、失败可重试）；本地音乐可按文件夹导入、按歌手 / 专辑浏览，自带 .lrc 与内嵌歌词。
- **同步歌词**：桌面在右侧展开、手机整页显示，当前行平滑居中，可手动浏览、点击定位；桌面还可以把歌词悬浮在所有窗口之上。
- **听感**：预加载下一首、可调淡入淡出、响度均衡、桌面均衡器、睡眠定时、最近播放、下一首播放与队列拖拽排序。
- **完整音源下载**：下载当前歌曲的最高可用音源，保留原格式，写入标签、封面和歌词，Rust 全流解码校验后才落盘；网易云无完整音源时可用 QQ 账号补源（严格核对同一录音）。
- **记住上次**：关闭时的歌曲、进度、所在歌单或歌手 / 专辑页、窗口位置与大小、歌词面板、音量，下次打开接着来；数据可一键备份与恢复。
- **桌面扩展**：托盘（菜单栏）控制与关闭到托盘、迷你播放器、全局快捷键、自选下载文件夹、界面缩放。
- **自动更新**：桌面版启动后静默下载已签名的新版本，准备好后一键重启完成。
...
**当前版本 0.9.7**（iOS 构建 90701，Android versionCode 9007）。macOS 需 15+ Apple Silicon；iOS 16+ 为开发签名真机版；Windows / Linux / Android 由 CI 构建。
...
- 导入本地音频后长期记住（读取标签与内嵌封面），本地歌曲可以收藏、加入本机歌单并与两平台在线歌曲混编；桌面按路径记住、可导入整个文件夹，手机把导入的文件复制到应用内。macOS 标题栏与内容区融合。
- 多选（桌面 ⌘/Ctrl、Shift 单击，或点「选择」）后批量下一首播放、加入队列、加入歌单、收藏、下载；歌单、收藏、本地、队列内可筛选与排序；在线歌单可复制为本机歌单；本机歌单封面可选第一首、四宫格或自选图片。
- 记住上次播放的歌曲、进度与关闭时所在的歌单；桌面版还记住窗口位置与大小、歌词面板开合、音量；三栏布局可拖拽调整列宽。
...
当前开发构建未做 Developer ID 签名和公证。请只运行可信来源的构建。桌面版启动后会静默下载已签名的新版本，准备好后提示「重启更新」，点击即完成更新，详见 [自更新](docs/SELF-UPDATE.md)；从源码自行构建的应用仍需重新构建并替换。根目录的 `打开 Ting.command` 优先打开当前版本 `Release/v<version>/macOS-AppleSilicon/` 中的应用，其次打开构建输出，**它不会自动重新编译**。
...
iOS 下载保存在应用 Documents/Ting，可在「文件 → 浏览 → 我的 iPhone / iPad → 听 · Ting → Ting」查看。移动端歌词在全屏面板显示，音量使用设备按键，搜索提交后收起键盘；触控按钮保留至少 44px 点击区域。Android 下载保存在应用专属目录，通过「查看下载」选取文件并调用系统保存／分享；卸载前请先导出需要保留的音频。Android 二维码使用原生分享，系统播放面板使用原生 MediaSession。Android 使用 Android Keystore 的不可导出密钥加密保存两个平台的凭据，应用重启后自动恢复；加密记录不参与系统备份或设备迁移，退出某个平台只清除该平台记录。Windows / Linux 使用系统凭据存储（Windows 凭据管理器 / Linux secret service）保存登录；macOS / iOS 使用钥匙串。
...
在「本地」页导入音频文件即可离线播放，不会上传本地文件。格式支持取决于系统 WebView；不同操作系统的解码能力可能不同。

桌面版（macOS / Windows / Linux）通过系统文件对话框导入，应用只记住文件路径并读取标签（标题、歌手、专辑、时长、内嵌封面），不复制音频；下次启动自动重新授权这些文件，被移动或删除的文件会标记「文件丢失」，可从歌曲菜单移出本地音乐库。本地歌曲可以收藏、加入本机歌单，和网易云 / QQ 歌曲混编；这些归属只保存在本机，不进入 iCloud 同步（路径在另一台设备上没有意义）。手机端导入仍只保持本次会话。

在线歌曲的收藏、队列、本机歌单只是元数据，不是离线音频缓存。搜索、扫码、云端播放、在线封面和歌词需要联网。已下载文件可通过「本地」页导入。

手机采用上方歌曲内容、底部紧凑播放区与导航；横屏（含平板横屏）分为列表和播放器两栏。桌面窗口保持紧凑竖向布局，拉宽到横向后同样自动切换为列表和播放器两栏，收窄即恢复。歌单详情页提供返回按钮，桌面可按 Esc、iOS 可从屏幕左缘右滑、Android 使用系统返回键或侧滑返回歌单列表。聚焦搜索时临时收起播放器以容纳软键盘；iOS 键盘弹出时按实际可见区域调整登录弹窗，输入框可在弹窗内滚动。各列表保留自己的滚动位置，新打开的歌单从顶部开始；点击「刷新」主动更新云歌单。Logo 在当前首页再次点击不会重载内容。
...
0.8.0 的审查修复继续保留，0.9.0 将 QQ 与下载迁到 Rust 并增加 iOS 真机构建。完整范围和验证边界见 [审查记录](docs/REVIEW.md) 与 [验证历史](docs/VALIDATION.md)。当前仍未完成公开商店发行（各平台均为开发 / 体验级签名）、Developer ID 公证和本机数据迁移工具。
